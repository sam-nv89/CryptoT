...
async function testMoralis() {
  ...
  for (const w of whales) {
    const url = `https://deep-index.moralis.io/api/v2.2/wallets/${w.address}/net-worth?chain=${w.chain}&exclude_spam=true&exclude_unverified_contracts=true`;
    try {
      const res = await fetch(url, {
        headers: { 'Accept': 'application/json', 'X-API-Key': apiKey }
      });
      const data = await res.json();
      console.log(`${w.name} Net Worth: $${data.total_networth_usd}`);
    } catch (e) {
      console.error(e);
    }
  }
