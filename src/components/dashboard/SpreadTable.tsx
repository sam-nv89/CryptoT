          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border/70">
                ...
                <th className="px-3 py-2.5 text-right text-xs text-text-muted">Спред $</th>
                <th className="px-3 py-2.5 text-right text-xs text-text-muted">Объём 24ч</th>
              </tr>
            </thead>
            <tbody>
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-border/70">
                  ...
                  <SortableHeader label="Спред $" sortKey="spreadAbsolute" currentSort={sort} onSort={handleSort} align="right" />
                  <SortableHeader label="Объём 24ч" sortKey="volume24h" currentSort={sort} onSort={handleSort} align="right" />
                </tr>
              </thead>
              <tbody>
                {displayed.map((spread, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-right">
                        <span className="mono-number text-text-secondary text-xs">
                          ${spread.spreadAbsolute < 0.01
                            ? spread.spreadAbsolute.toFixed(6)
                      ...
                      <td className="px-3 py-2 text-right">
                        <span className="mono-number text-text-muted text-xs">
                          {spread.volume24h >= 1_000_000
                            ? `$${(spread.volume24h / 1e6).toFixed(1)}M`
                            : spread.volume24h >= 1_000
                              ? `$${(spread.volume24h / 1e3).toFixed(0)}K`
                              : `$${spread.volume24h.toFixed(0)}`}
                        </span>
                      </td>
                    </tr>
                  );
